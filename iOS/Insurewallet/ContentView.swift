...
struct ContentView: View {
    ...
    @State private var editingEntry: InsurewalletEntry?
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Insurance Card Wallet")
            .toolbar {
...
enum EntryFormMode {
    case add
    case edit(InsurewalletEntry)
}

struct EntryFormView: View {
    let mode: EntryFormMode
    var onSave: (InsurewalletEntry) -> Void
        ...
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry: InsurewalletEntry
                        if case .edit(let existing) = mode {
                            entry = existing
                        } else {
                            entry = InsurewalletEntry(title: "")
                        }
                        entry.title = title.isEmpty ? "Untitled" : title
                        entry.date = date
                        entry.provider = field2
                        entry.policyNumber = field3
                        entry.note = note
                        onSave(entry)
                        dismiss()
                    }
            ...
            .onAppear {
                if case .edit(let existing) = mode {
                    ...
                    date = existing.date
                    field2 = existing.provider
                    field3 = existing.policyNumber
                    note = existing.note
                }
            }
        }
